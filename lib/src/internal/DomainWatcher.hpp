...
struct DomainWatcherRecord
{
    ...
    bool operator==( const DomainWatcherRecord &other ) const { return id == other.id && type == other.type; }
};
...
public:
    using Callback = std::function<void( const uuids::uuid &, WatcherType in_type )>;
    ...
    ///
    explicit DomainWatcher( const std::filesystem::path &in_domain, Callback in_callback );
    ...
    ///
    int16_t addFlow( const uuids::uuid &in_flowId, WatcherType in_type );
    ...
    ///
    int16_t removeFlow( const uuids::uuid &in_flowId, WatcherType in_type );
