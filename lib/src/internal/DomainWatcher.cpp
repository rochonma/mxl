...
DomainWatcher::DomainWatcher( const std::filesystem::path &in_domain, Callback in_callback )
    : _domain{ in_domain }, _callback{ std::move( in_callback ) }, _running{ true }
{
...
int16_t
DomainWatcher::addFlow( const uuids::uuid &in_flowId, WatcherType in_type )
{
...
int16_t
DomainWatcher::removeFlow( const uuids::uuid &in_flowId, WatcherType in_type )
{
    auto id = uuids::to_string( in_flowId );
