...
public:
    ...
    ///
    bool open( const std::string &in_path, bool in_create, AccessMode in_mode, size_t in_extraSize = 0 );
...
template <typename T>
bool
SharedMem<T>::open( const std::string &in_path, bool in_create, AccessMode in_mode, size_t in_payloadSize )
{
    if ( _fd != -1 )
    {
        // Already open
