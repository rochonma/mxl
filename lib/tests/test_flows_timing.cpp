...
TEST_CASE_PERSISTENT_FIXTURE(mxl::tests::mxlDomainFixture, "Video Flow : Wait for grain availability", "[mxl flows timing]")
{
    ...
    auto const flowId = uuids::to_string(fInfo.config.common.id);
    ...
    auto const readerGrainIndex = mxlGetCurrentIndex(&fInfo.config.common.grainRate);
    auto const frameDurationNs = 1000000000 * fInfo.config.common.grainRate.denominator / fInfo.config.common.grainRate.numerator;
