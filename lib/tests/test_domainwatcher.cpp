...
TEST_CASE("DomainWatcher triggers callback on file modifications", "[domainwatcher]")
{
    ...
    std::unique_lock<std::mutex> lock(eventMutex);
    bool gotEvent = eventCV.wait_for(lock, std::chrono::seconds(2), [&]{ 
        return eventCount.load() > initialCount; 
    });
    lock.unlock();
...
TEST_CASE("DomainWatcher error handling for invalid inputs", "[domainwatcher]")
{
    ...
    REQUIRE_THROWS_AS(
        watcher.addFlow(dummyId, mxl::lib::WatcherType::READER),
        std::runtime_error
    );
    ...
    REQUIRE_THROWS_AS(
        watcher.addFlow(noFileId, mxl::lib::WatcherType::WRITER),
        std::runtime_error
    );
...
TEST_CASE("DomainWatcher supports concurrent add/remove operations", "[domainwatcher]")
{
    ...
    const int iterations = 100;  // Reduced iterations for more predictable results
    ...
    std::thread adder([&]()
    {
        for (int i = 0; i < iterations; ++i)
        {
            try {
                ...
                if (result > 0) addCount++;
            } catch (...)
            {
                // Ignore exceptions from concurrent access
            }
            if (i % 10 == 0)
            { std::this_thread::yield(); }  // occasionally yield to other thread
        }
    });
    std::thread remover([&]()
    {
...
TEST_CASE("DomainWatcher ctor throws on invalid domain path", "[domainwatcher]")
{
    ...
    REQUIRE_THROWS_AS(
        mxl::lib::DomainWatcher(tmp, nullptr),
