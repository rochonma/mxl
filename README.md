...
*Note: the debug and trace log statements are statically excluded from the library at compilation time in release mode.*
...
Simple tool that loads a single v210 grain + NMOS Flow json file and repeats it at a specific grain rate as defined in the json file. The grain and json definition are stored in a zip file. The code expects grain.v210 and flow.json entries to exist in the root of the zip file.

```bash
mxl-clip-player
Usage: ./mxl-clip-player [OPTIONS]
...
Ideally this tool should use GStreamer to consume the output of a gstreamer pipeline and write them in MXL using the MXL sdk. The current implementation is just a throwaway tool useful to get started with MXL.
